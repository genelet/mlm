...
### 4.2) Auto Placement and Power Leg

On member portal, one can assign a member ID and a leg, as the default upline for new signups in the pyramid tree. This is called *Auto Placement*. If the leg is already been occupied, we will use her direct downline as the new upline, and the leg is kept the same.

For example, Mary assign John's left leg as her *Auto Placement*. If Henry joins and occupies John's left leg, the second new signup will be placed under Henry's left leg. And so on. Even Mary does not change the rule, the system will be smart enough to build Mary's one-sided line, called *power line* automatically.

In practice, all members will try to take advantage of a powerful upline's power line, so they need only to build up their weaker lines to accumulate points. Member should adjust her *auto placement* when necessary, so as to optimize the pairing bonus.

### 4.3) Customization

If developers want to develop new code, they should check out the [Tutorial](http://www.genelet.com/index.php/tutorial-perl/) and [Manual](http://www.genelet.com/index.php/2017/02/08/perl-development-manual/) of *Genelet*. It is a very powerful framework, yet has a short learning-curve to develop quickly.

If developers merely want to customize compensations, they can focus on programs under *lib/MLM/Income*. For example, if to add a new commission called *Bonus X*, what you may do like these:
- add new column *statusX* to database table *cron_1week*
- add new action name "*week1_x*" in *lib/MLM/Income/component.json*
- any associated parameter should be defined in *Custom* of *config.json*
- write new methods *is_week1_x*, *week1_x*, *done_week1_x* and *weekly_x* according to the commission plan, in *lib/MLM/Income/Model.pm*
- add it to right places in *run_cron*, *run_daily* and *run_all_tests* in *Model.pm*
