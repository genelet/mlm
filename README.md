...
### 4.2) Auto Placement and Power Line
...
### 4.4) Development in Java and GO Lang
...
- Create a new view directory, and have string *Template*, currently *SAMPLE_home/mlm/views* in *config.json* point to it.
...
```
- make changes as you like. In case, you may switch back to the original pages.

### 4.6) JSON API

Another useful feature of *Genelet* is that one gets alwawys the returned page in JSON by changing the tag name *en* to *json* in the URL. This could be used as API for other development such as mobile apps.
